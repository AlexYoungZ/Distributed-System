...
type ShardMaster struct {
	...
	DebugLog  bool
	lockStart time.Time // debug to find long time lock
	lockEnd   time.Time
	lockName  string
}
...
func (sm *ShardMaster) adjustConfig(config *Config) {
	if len(config.Groups) == 0 {
		...
		// evenly distribute avg num of shards over gid
	...
	LOOP:
		...
		for _, gid := range keys {
			...
			if count == avg {
				...
				// reduce othersShardsCount to 0
				// if still count > avg, then set to 0
				c := 0
				for i, val := range config.Shards {
					if val == gid {
				...
				// count < avg, possible no available position
				for i, val := range config.Shards {
					if count == avg {
						break
					}
		...
		// possible that each gid >= avg，have empty shard
		if lastGid != 0 {
			for i, val := range config.Shards {
				if val == 0 {
		...
		// each gid get max one， left extra gid
