	...
	for !rf.killed() {
		...
		if reply.Success {
			...
			if len(args.Entries) > 0 && args.Entries[len(args.Entries)-1].Term == rf.term {
				// only commit its own term index
